...
$unidad = "";
$v = $vm = $vM = $vcm = $vcM = "";
$rs = pg_query($link, filtrar_sql("select fecha_evento::date, COUNT(id_log_sensor), SUM(tiempo_puerta(id_log_sensor, fecha_evento)), SUM(tiempo_puerta(id_log_sensor, fecha_evento))/COUNT(id_log_sensor) from log_sensor where id_sensor = $sen and dato=2 group by fecha_evento::date order by fecha_evento::date asc ")); 
