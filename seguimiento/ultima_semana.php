...
if(isset($_REQUEST['id'])==false){ 
	Auditoria("En Seguimiento de Unidad Especifico Acceso Invalido Archivo Recorrido Ultima Semana",0);
...
//$dia[0] = filtrar_campo('date', 10, '2015-03-31');
...
$rs = pg_query($link, filtrar_sql("select count(id_log_gps) from log_gps where id_unidad = $id and fecha_hora_gps between '".$dia[7]." 00:00:00' and '".$dia[0]." 23:59:59'")); 
$rs = pg_fetch_array($rs);
if($rs[0]<1){ 
...
$rs = pg_query($link, filtrar_sql("select geo_posicion from log_gps where id_unidad = $id and fecha_hora_gps between '".$dia[7]." 00:00:00' and '".$dia[0]." 23:59:59' order by fecha_hora_gps desc limit 1")); $rs = pg_fetch_array($rs);
...
$rs = pg_query($link, filtrar_sql("select unidades.codigo_principal, velocidad_gps, geo_posicion, fecha_hora_gps, valor_max, valor_min, valor_critico_max, log_gps.dconfunid, 
(fecha_hora_gps)::time, sentido, 
...
from unidades, log_gps where unidades.id_unidad = $id and log_gps.id_unidad = $id and  fecha_hora_gps between '".$dia[$k]." 00:00:00' and '".$dia[$k]." 23:59:59' order by log_gps.fecha_hora_gps asc "));
...
$data[$k]['punto'] .= "[$lat, $lng, '".$r[7].' '.$r[0]."', $tipo, 'Fecha y Hora: ".date1($r[3])."<br/>".$r[7].' '.$r[0]."<br/>Velocidad: ".(1*$r[1])." Km/Hr.<br/>Ref. ".(1*$r[5])." Km/Hr - ".(1*$r[4])." Km/Hr<br/>'],";

if($r[10]=='t'){       /*Mañana*/  $data[$k]['path']['man'] .= cordenadas3($r[2]);
} else if($r[11]=='t'){ /*Tarde*/  $data[$k]['path']['tar'] .= cordenadas3($r[2]);
} else if($r[12]=='t'){ /*Noche*/  $data[$k]['path']['noc'] .= cordenadas3($r[2]); }
