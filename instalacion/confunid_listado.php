...
if(isset($_REQUEST['eli'])){ 
	...
	if($rs){ 
		...
		Auditoria("En Instalacion Configuracion de Unidad Fue Eliminada ",$id); 
		$_SESSION['mensaje3']="Configuracion de Unidad Fue Eliminada";
	} else { 
		$_SESSION['mensaje1']="No se logro eliminar la Configuracion de Unidad";
		Auditoria("En Instalacion Problema al eliminar la Configuracion de Unidad Error: ".pg_last_error($link),$id);
	}
	
} else { 
	Auditoria("En Instalacion Accedio al Listado Configuracion de Unidad",0);
}
...
<a href="confunid_agregar.php" target="cfunid" style="margin-left:380px; color:#2DB6FF;"><img src="../img/plus.png" width="15" height="15" title="Agregar Nuevo Conf. de Unidad" style="margin-right:7px;" /> Agregar Conf. de Unidad</a> </div>
...
<img src="../img/cambios.png" width="15" height="15"  title="Seleccionar Conf. de Unidad" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="location.href='confunid_ver.php?id=<?php echo $r[0];?>'"/>
</div></td>
<td><?php echo $r[2];?></td>
<td><?php echo $r[1];?></td>
</tr>
...
<?php if(isset($_SESSION['instalacion']['cfunid'])) { 
	...
	echo "<script>window.open('confunid_ver.php','cfunid');</script>";
} else { 
