...
if(isset($_REQUEST['conf'])){ $_SESSION['confunid']=filtrar_campo('int', 6,$_REQUEST['conf']); }
...
if(isset($_POST['guardar'])){  
	...
	if(empty($cod)){ $_SESSION['mensaje1']="Debe indicar el tipo de unidad";
		...
		$rs = pg_query($link, filtrar_sql("update confunid set nombre='$nom', codigo_principal='$cod', n_configuracion_01='$conf1', n_configuracion_02='$conf2', n_configuracion_03='$conf3', n_configuracion_04='$conf4' where id_confunid = ".$_SESSION['confunid']));
		if($rs){
			Auditoria("En Instalacion Actualizo Los Datos de la Configuracion de Unidades: $nom",$_SESSION['instalacion']['cfunid']);
	...
	if($r==false || $r<1){ 
		...
		Auditoria("En Instalacion Configuración de Unidad No Identificada ",$_SESSION['confunid']);
		...
		$conf3=$rs[6]; 
		$conf4=$rs[7];
		Auditoria("En Instalacion Accedio a Editar Configuracion de unidades: $cod ",$_SESSION['confunid']);
	}

} else { 
	$_SESSION['mensaje1']="No se identifico la configuración de la unidad";
	Auditoria("En Instalacion Configuración de Unidad No Identificada ",0);
	unset($_SESSION['confunid']);
	header("location: confunid_listado.php");
	exit();
}
