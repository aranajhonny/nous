...
if(isset($_REQUEST['tipoperm'])){ $_SESSION['confperm']['tperm']=filtrar_campo('int',6,$_REQUEST['tipoperm']); }
...
if(empty($resp)){ $_SESSION['mensaje1']="Debe seleccionar un Responsable General";
...
$_SESSION['treq']['req'][$i][2]=filtrar_campo('string',30,$_POST["est_".$i]); // Estatus 
	...
	$qs = pg_query($link, filtrar_sql("update tipo_permisos set id_responsable_general = $resp, dias_gestion = $dias, nombre = '$nom', cant_doc = $cant_doc, tipo_aviso='$aviso', tipo_msj='$msj', esc = $esc, id_clasperm = $clas where id_tipo_permiso = ".$_SESSION['confperm']['tperm']));
	if($qs){
	Auditoria("Actualizo Tipo Permiso: $nom ",$_SESSION['confperm']['tperm']);

// -----------------------------------------------------------------------------
for($i=1; $i<=$CantItems; $i++){ 
	if(empty($_SESSION['treq']['req'][$i][1])==false){ 
		if($_SESSION['treq']['req'][$i][0]==0){ 
			pg_query($link, filtrar_sql("insert into req_tipperm(id_tipo_permiso, descripcion, estatus) values (".$_SESSION['confperm']['tperm'].", '".$_SESSION['treq']['req'][$i][1]."', 'Activo')"));
		} else { 
...
// -----------------------------------------------------------------------------
		...
		header("location: tperm_listado.php");
		exit();
	} else { 
...
} else if(isset($_SESSION['confperm']['tperm'])){
$rs = pg_query($link, filtrar_sql("select * from tipo_permisos where id_tipo_permiso=".$_SESSION['confperm']['tperm']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico El Tipo de Permiso";
	Auditoria("Tipo de Permiso No Identificado ",$_SESSION['confperm']['tperm']);
	...
	$esc = $rs[8];
	$clas = $rs[9];
	Auditoria("Accedio Al Modulo Editar Tipo de Permiso: $nom",$_SESSION['confperm']['tperm']);
	
	$req = array();
	$rs = pg_query($link, filtrar_sql("select id_reqtipperm, descripcion, estatus from req_tipperm where id_tipo_permiso = ".$_SESSION['confperm']['tperm']." order by descripcion asc "));
	$r = pg_num_rows($rs);
	if($r!=false && $r>0){ $i=0; 
...
<div class="header">Editar Datos del Tipo de Permiso</div>
<form name="editar" method="post" action="tperm_editar.php" onsubmit="return validar();">
<fieldset>
<div class="fuelux">
...
<?php $rs = pg_query($link, filtrar_sql("select id, nom from clasperm where id_cliente = ".$_SESSION['confperm']['cli']." order by nom asc")); 
$r = pg_num_rows($rs);
...
<?php $rs = pg_query($link, filtrar_sql("select id_personal, ci, nombre from personal where id_cliente = ".$_SESSION['confperm']['cli']." order by ci asc")); 
$r = pg_num_rows($rs);
if($r!=false && $r>0){ while($r = pg_fetch_array($rs)){?>    
...
<div class="row">
<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
<input type="button" name="volver" value="Volver" class="btn btn-info btn-block" onclick="location.href='tperm_listado.php'"/></div>
<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
