...
if(isset($_REQUEST['cli'])){
	...
	if($r==false || $r<1){ 
		...
		Auditoria("Cliente No Identificado ",$id);
		...
		$_SESSION['confperm']['cli'] = $rs[0];
		$_SESSION['confperm']['paso'] = 1;
		$rs = pg_query($link, filtrar_sql("select descripcion from tipo_clientes where id_tipo_cliente = $tipo"));
		$rs = pg_fetch_array($rs);
		$tipo = $rs[0];
		Auditoria("Accedio a Ver Datos del Cliente: $rif $razon",$_SESSION['confperm']['cli']);
	}

} else if(isset($_SESSION['confperm']['cli'])){
	$rs = pg_query($link, filtrar_sql("select * from clientes where id_cliente = ".$_SESSION['confperm']['cli']));
	$r = pg_num_rows($rs);
	if($r==false || $r<1){ 
		$_SESSION['mensaje1']="No se identifico el Cliente";
		Auditoria("Cliente No Identificado ",$_SESSION['confperm']['cli']);
		unset($_SESSION['confperm']['cli']);
		header("location: cliente_listado.php");
		exit();
	} else { 
		$rs = pg_fetch_array($rs);
	...
	$_SESSION['mensaje1']="No se identifico el cliente";
	Auditoria("Cliente No Identificado ",0);
	unset($_SESSION['confperm']['cli']);
	header("location: cliente_listado.php");
	exit();
}
...
<input id="tipo" name="tipo" type="text" placeholder="Tipo de Cliente" class="form-control" maxlength="200" value="<?php echo $tipo;?>" readonly="readonly"/></div>
...
<?php if( isset($_SESSION['confperm']['cli']) ) {  
	echo "<script>window.open('confunid_listado.php','confs');</script>";
}?>
