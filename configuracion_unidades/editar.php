...
if(isset($_REQUEST['confunid'])){ $_SESSION['confunid']=filtrar_campo('int', 6,$_REQUEST['confunid']); }

if(isset($_POST['guardar'])){ 
$cli = filtrar_campo('int', 6,$_POST['cli']); 
filtrar_campo('string', 50, $cod = $_POST['cod']); 
filtrar_campo('string', 100, $nom = $_POST['nom']); 
filtrar_campo('string', 50, $conf4 = $_POST['conf4']);
filtrar_campo('string', 50, $conf1 = $_POST['conf1']);  
filtrar_campo('string', 50, $conf2 = $_POST['conf2']);   
filtrar_campo('string', 50, $conf3 = $_POST['conf3']); 
...
} else if(in_array(211,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";

} else { // si validar 

	$rs = pg_query($link, filtrar_sql("update confunid set id_cliente = $cli, nombre='$nom', codigo_principal='$cod', n_configuracion_01='$conf1', n_configuracion_02='$conf2', n_configuracion_03='$conf3', n_configuracion_04='$conf4' where id_confunid = ".$_SESSION['confunid']));
	if($rs){ 
		Auditoria("Actualizo Configuracion de unidades: $cod ",$_SESSION['confunid']);
		$_SESSION['mensaje3']="Configuración de Unidad Editada";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro editar la configuración de la unidad";
		Auditoria("Problema al Actualizar Configuración de Unidad Error: ",$_SESSION['confunid']);
	}

} // si validar

} else if(isset($_SESSION['confunid'])){
$rs = pg_query($link, filtrar_sql("select * from confunid where id_confunid = ".$_SESSION['confunid']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico La Configuración de Unidad";
	Auditoria("Configuración de Unidad No Identificada ",$_SESSION['confunid']);
	unset($_SESSION['confunid']);
	header("location: listado.php");
	exit();
} else {
	...
	$conf3=$rs[6]; 
	$conf4=$rs[7];
Auditoria("Accedio Al Modulo Editar Configuracion de unidades: $cod",$_SESSION['confunid']);
}
...
<div>
<?php 
$rs = pg_query($link,"select rif, razon_social from clientes where id_cliente = ".$_SESSION['cfcli']); $rs = pg_fetch_array($rs); ?>
<input id="cli" name="cli" type="hidden" value="<?php echo $_SESSION['cfcli'];?>" readonly="readonly"/><input id="dcli" name="dcli" type="text" placeholder="Cliente Actual" class="form-control" value="<?php echo $rs[0]." ".$rs[1];?>" readonly="readonly" /></div></div>
