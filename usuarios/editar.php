...
} else if(in_array(241,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 

if(strcmp($est,"Activo")==0) $tmp="TRUE"; else $tmp="FALSE";

$rs = pg_query($link, filtrar_sql("update usuarios set nom='$nom', id_tipou=$tipou, est=$tmp where id_usuario = ".$_SESSION['usuario_personal']));
if($rs){ 

Auditoria("Actualizo Usuario: $nom",$_SESSION['usuario_personal']);

	pg_query($link, filtrar_sql("delete from sys_acl where id_usu = ".$_SESSION['usuario_personal']));
	for($i=0; $i<1000; $i++){ 
		if(isset($_POST["chk_".$i])){ 
			$rs = pg_query($link, filtrar_sql("insert into sys_acl(id_acc,id_usu,permiso)values(".$_POST["chk_".$i].",".$_SESSION['usuario_personal'].",TRUE);"));
		}
	}
	$_SESSION['mensaje3'] = "Usuario Editado";
	header("location: listado.php");
	exit();
} else { 
	$_SESSION['mensaje1']="No se logro editar el usuario";
		Auditoria("Problema al actualizar El Usuario Error: ".pg_last_error($link),$_SESSION['usuario_personal']);
}

} // si validar 

} else if(isset($_SESSION['usuario_personal'])){ 
$rs = pg_query($link, filtrar_sql("select * from usuarios where id_usuario = ".$_SESSION['usuario_personal'])); 
	...
	if($r!=false && $r>0){ 
		...
		Auditoria("Accedio Al Modulo Editar Usuario: $nom",$_SESSION['usuario_personal']);
	} else { 
...
<div class="header">Editar Usuario<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
...
$ws = pg_query($link, filtrar_sql("select count(id_acl) from sys_acl where id_usu = ".$_SESSION['usuario_personal']." and id_acc = ".$tmp[$i])); $ws = pg_fetch_array($ws); ?><td align="center"><input type="checkbox" name="chk_<?php echo $t.$i;?>" class="select_<?php echo $i;?>" value="<?php echo $tmp[$i];?>" <?php if($ws[0]>0) echo "checked";?> /></td>
<?php } else { ?> 
