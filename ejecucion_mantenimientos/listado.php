...
$_SESSION['acc']['mod'] = 50;
$_SESSION['acc']['form'] = 160;
include("../complementos/permisos.php");


Auditoria("Accedio Al Modulo de Ejecución de Mantenimientos",0);
unset($_SESSION['ejemant']);
...
<div class="header">Listado Ejecuciones de Mantenimiento
...
$rs = pg_query("select fe, ejemant.id_planmant, confunid.codigo_principal, unidades.codigo_principal, total, ejemant.obs, id_ejemant from ejemant, confunid, unidades where (confunid.id_cliente=$c and unidades.id_cliente=$c) and (confunid.id_confunid=$t or $t < 1) and ejemant.id_unidad = unidades.id_unidad and unidades.id_confunid = confunid.id_confunid order by fe, confunid.codigo_principal, unidades.codigo_principal desc "); 
$r = pg_num_rows($rs);
if($r!=false && $r>0){ while($r = pg_fetch_array($rs)){   

if($r[1]==0){ $plan=" - - "; } else { 
	$qs=pg_query("select descripcion from planmant where id_planmant = ".$r[1]);
	$qs=pg_fetch_array($qs);
	$plan = $qs[0]; }?>    
<tr>
<td><div class=" info-tooltip" style="width:100px;">

<?php if ( in_array(429,$_SESSION['acl']) ){ ?>
<img src="../img/search.png" width="15" height="15"  title="Ver datos de la Ejecución del Mantenimiento"  rel="tooltip" data-placement="right" onclick="window.open('ver.php?eje=<?php echo $r[6];?>','_blank');"/>
<?php } ?>

<?php if ( in_array(427,$_SESSION['acl']) ){ ?>
<img src="../img/plus.png" width="15" height="15" title="Agregar Nueva Ejecucuón de Mantenimiento" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="window.open('agregar.php','_blank');" />
<?php } ?>
...
<td></td>
<td></td>
<td><?php if ( in_array(427,$_SESSION['acl']) ){ ?>
<img src="../img/plus.png" width="15" height="15" title="Agregar Nueva Ejecucuón de Mantenimiento" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="window.open('agregar.php','_blank');" />
<?php } ?></td>
<td></td>
</tr>
...
<tfoot><tr>
<th><input type="text" name="acc" id="acc" style="width:0px;height:0px;border:none;visibility:hidden;"/></th>
