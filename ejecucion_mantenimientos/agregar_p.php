...
$_SESSION['acc']['mod'] = 50;
$_SESSION['acc']['form'] = 158;
include("../complementos/permisos.php");


if(isset($_REQUEST['progmant'])){ 
unset($_SESSION['detplan']);

$rs = pg_query("select id_progmant, planmant.id_planmant, id_unidad, id_confunid from progmant, planmant_unidades, planmant where id_planmantunid = id_planmant_unidad and planmant_unidades.id_planmant = planmant.id_planmant and id_progmant = ".$_REQUEST['progmant']);
	$rs = pg_fetch_array($rs);
	$_SESSION['progmant']['id']=$rs[0];
	$_SESSION['progmant']['planmant']=$rs[1];
	$_SESSION['progmant']['unidad']=$rs[2];
	$_SESSION['progmant']['confunid']=$rs[3];
	$_SESSION['progmant']['cli']=3;

$rs = pg_query("select id_detplanmant, id_composicion, id_provserv, UPPER(descripcion) from det_planmant where id_planmant = ".$_SESSION['progmant']['planmant']." order by id_detplanmant asc ");
$r = pg_num_rows($rs);
if($r!=false && $r>0){ $i=0; 
	while($r = pg_fetch_array($rs)){  	

		$_SESSION['detplan'][$i][0] = $r[3]; // etiqueta
		$_SESSION['detplan'][$i][1] = "off"; // valor del chk
		$_SESSION['detplan'][$i][2] = ""; // observacion
		$_SESSION['detplan'][$i][3] = 0; // tipo de detalle
		$_SESSION['detplan'][$i][4] = $r[1]; // id composicion
		$i++; 
	} 
}
...
for($i=1; $i<=$total_fact; $i++){
	if(empty($_POST["nro_$i"])==false) $facturas .= "(:::, ".$_POST["prov_$i"].", '".$_POST["nro_$i"]."', '".date2($_POST["ff_$i"])."', '".date2($_POST["fr_$i"])."', ".$_POST["monto_$i"].", NULL, ''),";
}
...
if($cant_fact==0){  $_SESSION['mensaje1']="Debe Agregar Al Menos Una Factura"; 
...
} else if(in_array(426,$_SESSION['acl'])==false){$_SESSION['mensaje']= "no posee permiso para guardar este registro";
} else { // si validar


if(pg_query("insert into ejemant(id_progmant, id_planmant, id_unidad, id_confunid, total, fe, obs) values (".$_SESSION['progmant']['id'].", ".$_SESSION['progmant']['planmant'].", ".$_SESSION['progmant']['unidad'].", ".$_SESSION['progmant']['confunid'].", ".($costo*1).", '$fe', '$obs')")){ 
	...
	$rs = pg_query("select descripcion from planmant where id_planmant = ".$_SESSION['progmant']['planmant']); $rs = pg_fetch_array($rs); $mantenimiento = $rs[0];
	
	
	Auditoria("Agrego Ejecución de Mantenimiento a la Unidad: $unidad Segun Plan de Mantenimiento: $mantenimiento en Fecha ".date1($fe), $id);
...
<div class="row">
<form name="agregar" method="post" action="agregar_p.php" onsubmit="return validar();">
...
<script>
var ListProv = "";
var code = <?php echo $_SESSION['progmant']['cli']; ?>;
...
var ListComp = "";
$.get("det_composiciones.php?conf="+<?php echo $_SESSION['progmant']['confunid']; ?>, 					
	function(resultado){ 
		if(resultado == false){  
