...
for($i=0; $i<7; $i++){ 
$aten = $sina = $total = 0;
$rs = pg_query($link, filtrar_sql("select count(id_noti) from notificaciones, alarmas, unidades where  notificaciones.id_alarma = alarmas.id_alarma and 
unidades.id_unidad = alarmas.id_unidad and 
(alarmas.id_cliente=$c and unidades.id_cliente=$c) and 
((unidades.id_area=$a or $a < 1) and (unidades.id_zona=$z or $z < 1) and (unidades.id_confunid=$t or $t < 1)) and confirmacion = true and fecha_hora_enviado between '".$dias[$i]." 00:00:00' and '".$dias[$i]." 23:59:59'"));
$rs = pg_fetch_array($rs); $aten = $rs[0];

$rs = pg_query($link, filtrar_sql("select count(id_noti) from notificaciones, alarmas, unidades where  notificaciones.id_alarma = alarmas.id_alarma and unidades.id_unidad = alarmas.id_unidad and (alarmas.id_cliente=$c and unidades.id_cliente=$c) and ((unidades.id_area=$a or $a < 1) and (unidades.id_zona=$z or $z < 1) and (unidades.id_confunid=$t or $t < 1)) and confirmacion = false and fecha_hora_enviado between '".$dias[$i]." 00:00:00' and '".$dias[$i]." 23:59:59'"));
...
<title>Notificaciones Atendidas - Ultimos 7 Días</title>
...
$(function () {
        $('#container').highcharts({
            ...
            title: {
                text: 'Notificaciones Atendidas<br/>Ultimos 7 Días'
            },
            xAxis: {
