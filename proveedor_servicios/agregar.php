...
} else if(in_array(95,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else
{ // si validar 

	$rs = pg_query($link, filtrar_sql("insert into provserv(id_cliente, rif, nombre_prov, telefono, contacto, email, direccion) values($cli, '$rif', '$razon', '$tlf', '$contacto', '$correo', '$dir')"));
	if($rs){ 
	
		$rs = pg_query($link, filtrar_sql("select max(id_provserv) from provserv "));
		$rs = pg_fetch_array($rs);
		Auditoria("Agrego Proveedor de Servicios: $rif $razon",$rs[0]);
		
		$_SESSION['mensaje3']="Proveedor de Servicio Agregado";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro agregar el Proveedor de Servicio";
		Auditoria("Problema al registrar El Proveedor de Servicio Error: ".pg_last_error($link),0);
	}

} // si validar
} else { 
	$cli = $dir = $rif = $razon = $correo = $contacto = $tlf = "";
	Auditoria("Accedio Al Modulo Agregar Proveedor de Servicios",0);
}
...
<div class="header">Agregar Proveedor de Servicio<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
