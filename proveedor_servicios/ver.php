...
$_SESSION['acc']['mod'] = 44;
$_SESSION['acc']['form'] = 100;
include("../complementos/permisos.php");

if(isset($_REQUEST['provserv'])){ $_SESSION['provserv']=filtrar_campo('int', 6, $_REQUEST['provserv']); }

if(isset($_SESSION['provserv'])){
$rs = pg_query($link, filtrar_sql("select provserv.*, provserv.rif, razon_social from provserv, clientes where provserv.id_cliente = clientes.id_cliente and id_provserv = ".$_SESSION['provserv']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	...
	$contacto = $rs[2]; $correo = $rs[4]; $dir = $rs[6];  $cli = $rs[8]." ".$rs[9];
	Auditoria("Accedio Al Modulo Ver Proveedor: $rif $razon",$_SESSION['provserv']);
}
...
<div class="header">Ver Proveedor de Servicio<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
...
<input id="dir" name="dir" type="text" placeholder="Dirección" class="form-control" value="<?php echo $dir;?>" readonly="readonly" /></div>
