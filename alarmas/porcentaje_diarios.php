...
for($i=0; $i<7; $i++){ 
$aten = $sina = $total = 0;
$rs = pg_query($link, filtrar_sql("select count(id_alarma) from alarmas, unidades where unidades.id_unidad = alarmas.id_unidad and (alarmas.id_cliente=$c and unidades.id_cliente=$c) and ((alarmas.id_area=$a or $a < 1) and (alarmas.id_zona=$z or $z < 1) and (alarmas.id_confunid=$t or $t < 1)) and id_estatus_servicio = 29 and fecha_evento between '".$dias[$i]." 00:00:00' and '".$dias[$i]." 23:59:59'"));
$rs = pg_fetch_array($rs); $aten = $rs[0];

$rs = pg_query($link, filtrar_sql("select count(id_alarma) from alarmas where id_estatus_servicio = 31 and fecha_evento between '".$dias[$i]." 00:00:00' and '".$dias[$i]." 23:59:59'"));
...
<div id="container" style="min-width: 310px; max-width: 800px; height: 400px; width:400px; margin: 0 auto"></div>
	</body>
</html>
