...
$_SESSION['acc']['mod'] = 43;
$_SESSION['acc']['form'] = 95;
include("../complementos/permisos.php");

if(isset($_REQUEST['comp'])){ $_SESSION['composicion']=filtrar_campo('int',6,$_REQUEST['comp']); }

if(isset($_POST['guardar'])){ 
$nom = filtrar_campo('string', 30, $_POST['nom']); 
$des = filtrar_campo('string', 200, $_POST['des']); 
$dep = filtrar_campo('int', 6, $_POST['dep']);
$confunid = filtrar_campo('int', 6, $_POST['confunid']);
...
} else if(empty($confunid)){ $_SESSION['mensaje1']="Debe seleccionar un Tipo Unidad";
} else if(in_array(234,$_SESSION['acl'])==false){$_SESSION['mensaje']= "no posee permiso para guardar este registro";

} else { // si validar 

	$rs = pg_query($link, filtrar_sql("update composiciones set nombre='$nom', descripcion='$des', id_confunid=$confunid, id_dependencia=$dep where id_composicion = ".$_SESSION['composicion']));
	if($rs){
		Auditoria("Actualizo Composición de Unidades: $des",$_SESSION['composicion']); 
		$_SESSION['mensaje3']="Composición Editada";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro editar la composición";
		Auditoria("Problema al actualizar Composición de Unidad Error: ".pg_last_error($link),$_SESSION['composicion']);
	}

} // si validar

} else if(isset($_SESSION['composicion'])){
$rs=pg_query($link, filtrar_sql("select * from composiciones where id_composicion = ".$_SESSION['composicion']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico La Composición de Unidad";
	Auditoria("Composición de Unidad No Identificada ",$_SESSION['composicion']);
	unset($_SESSION['composicion']);
	header("location: listado.php");
	exit();
} else { 
	...
	$nom = $rs[3]; 
	$dep = $rs[4];
Auditoria("Accedio Al Modulo Editar Composición de Unidades: $des",$_SESSION['composicion']);
}

} else { 
	$_SESSION['mensaje1']="No se identifico la composición";
	Auditoria("Composición de Unidad No Identificada",$_SESSION['composicion']);
	unset($_SESSION['composicion']);
	header("location: listado.php");
	exit();
...
<div class="header">Editar Composición<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
...
<div><select id="confunid" name="confunid" class="selectpicker">
<option value="0" selected="selected">Seleccione un Tipo Unidad</option>
<?php $rs = pg_query($link, filtrar_sql("select id_confunid, codigo_principal from confunid 
order by codigo_principal asc"));
$r = pg_num_rows($rs);
if($r!=false && $r>0){ while($r = pg_fetch_array($rs)){?>    
<option value="<?php echo $r[0];?>" <?php if($confunid==$r[0]) echo "selected";?> ><?php echo $r[1];?></option> 
<?php } } ?>    
</select></div>
</div>
...
val = false;
	...
	} else if(document.getElementById('confunid').value=="0"){ 
		mensaje("Debe seleccionar un Tipo Unidad",1);
