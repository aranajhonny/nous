...
$_SESSION['acc']['mod'] = 68;
$_SESSION['acc']['form'] = 178;
include("../complementos/permisos.php");

if(isset($_REQUEST['clas'])){ $_SESSION['clasperm']=filtrar_campo('int', 6, $_REQUEST['clas']); }
...
} else if(in_array(453,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 

	$rs = pg_query($link, filtrar_sql("update clasperm set nom='$nom', id_resp=$resp where id = ".$_SESSION['clasperm']));
	if($rs){ 
		Auditoria("Actualizo Clasificación de Permiso: $nom",$_SESSION['clasperm']);
...
} else if(isset($_SESSION['clasperm'])){
$rs = pg_query($link, filtrar_sql("select * from clasperm where id=".$_SESSION['clasperm']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico La Clasificación de Permiso";
	Auditoria("Clasificación de Permiso No Identificada ",$_SESSION['clasperm']);
	unset($_SESSION['clasperm']);
	header("location: listado.php");
	exit();
} else { 
	$rs = pg_fetch_array($rs);
	$nom = $rs[1]; 
	$_SESSION['clasperm_cliente'] = $rs[2]; 
	$resp = $rs[3];
	Auditoria("Accedio Al Modulo Editar Clasificación de Permiso: $nom",$_SESSION['clasperm']);
}
...
<li><a href="#">Clasificación de Permiso</a></li>
...
<div class="header">Editar Clasificación de Permiso<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
<form name="agregar" method="post" action="editar.php" onsubmit="return validar();">
<fieldset>

<div class="form-group"><label>Nombre</label>
<input id="nom" name="nom" type="text" placeholder="Nombre de la Clasificación" class="form-control" maxlength="60" value="<?php echo $nom;?>" onkeypress="return permite(event,'todo')" onkeyup="mayu(this)" /></div>

<?php $cli = "";
$rs = pg_query($link, filtrar_sql("select rif, razon_social from clientes where id_cliente = ".$_SESSION['clasperm_cliente'])); $rs = pg_fetch_array($rs); $cli = $rs[0]." ".$rs[1]; ?>
...
<?php $rs = pg_query($link, filtrar_sql("select id_personal, ci, nombre from personal where id_cliente = ".$_SESSION['clasperm_cliente']." order by ci asc ") ); 
$r = pg_num_rows($rs);
...
val = false;
	if(document.getElementById('nom').value.length<3){ 
		...
		mensaje("Debe seleccionar un responsable de la Clasificación",1);
