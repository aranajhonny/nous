...
} else if(in_array(84,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 


	$rs = pg_query($link, filtrar_sql("insert into unidmed(id_magnitud, nombre) values($mag, '$nom')"));
	if($rs){ 
		$rs = pg_query($link, filtrar_sql("select max(id_unidmed) from unidmed"));
		$rs = pg_fetch_array($rs);
		Auditoria("Agrego Unidad de Medida : $nom",$rs[0]);
		$_SESSION['mensaje3']="Unidad de Medida Agregada";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro agregar la Unidad de Medida";
		Auditoria("Problema al registrar La Unidad de Medida Error: ".pg_last_error($link),0);
	}

} // si validar
} else { 
	$mag = $nom = "";
	Auditoria("Accedio Al Modulo Agregar Unidades de Medidas",0);
}
...
<div class="header">Agregar Unidad de Medida<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
<form name="agregar" method="post" action="agregar.php" onsubmit="return validar();">
<fieldset>

<div class="form-group"><label>Descripción</label>
<input id="nom" name="nom" type="text" placeholder="Breve Descripción de la Unidad de Medida" class="form-control" maxlength="60" value="<?php echo $nom;?>" onkeypress="return permite(event,'todo')" onkeyup="mayu(this)" /><p class="help-block">Ejemplo: Seg, Mtr, Mts/Seg, etc...</p></div>
