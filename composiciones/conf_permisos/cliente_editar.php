...
if(empty($rif)){ $_SESSION['mensaje1']="Debe indicar el numero del RIF";
	...
	if($rs[0]>0){ 
		...
		if($rs){ 
			Auditoria("Actualizo Los Datos del Cliente: $rif $razon",$_SESSION['aux']);
	...
	if($r==false || $r<1){ 
		...
		Auditoria("Cliente No Identificado ",$_SESSION['aux']);
		unset($_SESSION['aux']);
		header("location: cliente_listado.php?limpiar=true");
		exit();
	} else { 
		...
		$contacto = $rs[6]; 
		$correo = $rs[7];
		Auditoria("Accedio a Editar Cliente: $rif $razon ",$_SESSION['aux']);
	}
