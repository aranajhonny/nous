...
if(isset($_REQUEST['pers'])){ $_SESSION['confperm']['pers']=filtrar_campo('int', 6, $_REQUEST['pers']); }
...
$rs = pg_query($link, filtrar_sql("select count(id_personal) from personal where ci='$ci' and id_personal <> ".$_SESSION['confperm']['pers']));
$rs = pg_fetch_array($rs);
if($rs[0]>0){ 
	$_SESSION['mensaje1']="numero de CI ya registrado";
} else { // si rif 

	$rs = pg_query($link, filtrar_sql("update personal set ci='$ci', nombre='$nom', telefono='$tlf', email='$correo', id_cargo=$cargo, id_area=$area, id_zona=$zona, tipo_aviso='$aviso' where id_personal = ".$_SESSION['confperm']['pers']));
	if($rs==true || $rs>0){ 
		Auditoria("Actualizo Los Datos del Personal: $ci $nom",$_SESSION['confperm']['pers']);
...
} else if(isset($_SESSION['confperm']['pers'])){
$rs = pg_query($link, filtrar_sql("select * from personal where id_personal = ".$_SESSION['confperm']['pers']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico El Personal";
	Auditoria("Personal No Identificado ",$_SESSION['confperm']['pers']);
	header("location: personal_listado.php");
	exit();
} else {
	$rs = pg_fetch_array($rs);
	...
	Auditoria("Accedio a Editar Datos del Personal: $ci $nom ",$_SESSION['confperm']['pers']);
}
