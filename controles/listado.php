...
$_SESSION['acc']['mod'] = 21;
$_SESSION['acc']['form'] = 37;
include("../complementos/permisos.php");

Auditoria("Accedio Al Modulo de Control Listado",0);

unset($_SESSION['control']);
unset($_SESSION['control_cliente']);
unset($_SESSION['control_magnitud']);
...
<div class="header">Listado Controles de Sensor
...
$rs = pg_query($link, filtrar_sql("select id_control, controles.nombre, tiempo_activacion_min, tiempo_activacion_max, val_minimo, val_maximo, valor_critico_min, valor_critico_max, unidmed.nombre from controles, unimedcli, unidmed where (controles.id_cliente = $c or $c = -1) and controles.id_unimedcli = unimedcli.id_unimedcli and unimedcli.id_unidmed = unidmed.id_unidmed order by controles.nombre asc ")); 
$r = pg_num_rows($rs);
if($r!=false && $r>0){ while($r = pg_fetch_array($rs)){ ?>    
<tr>
<td><div class=" info-tooltip">

<?php if ( in_array(326,$_SESSION['acl']) ){ ?>
<img src="../img/search.png" width="15" height="15"  title="Ver datos del Control"  rel="tooltip" data-placement="right" onclick="location.href='ver.php?control=<?php echo $r[0];?>'"/>
<?php } ?>

<?php if ( in_array(220,$_SESSION['acl']) ){ ?>
<img src="../img/pencil.png" width="15" height="15" title="Editar Datos del Control" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="location.href='editar.php?control=<?php echo $r[0];?>'" />
<?php } ?>

<?php if ( in_array(80,$_SESSION['acl']) ){ ?>
<img src="../img/plus.png" width="15" height="15" title="Agregar Nuevo Control" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="location.href='agregar.php'" />
<?php } ?>

</div></td>
<td><?php echo $r[1];?></td>
<td><?php echo $r[2]." / ".$r[3]." Min";?></td>
<td><?php echo (1*$r[4])." / ".(1*$r[5])." ".(1*$r[8]);?></td>
<td><?php echo (1*$r[6])." / ".(1*$r[7])." ".(1*$r[8]);?></td>
</tr>
<?php } } ?>    
	</tbody>
</table>
    ...
    return {
        init: function () {
            var oTable = $('#managed-table').dataTable({
				...
				"oTableTools": {
                    "aButtons": [
{ "sExtends": "copy","mColumns": [1,2,3,4] },
{ "sExtends": "csv", "mColumns": [1,2,3,4] },
{ "sExtends": "pdf", "mColumns": [1,2,3,4] },
{ "sExtends": "xls", "mColumns": [1,2,3,4] }
                    ],
