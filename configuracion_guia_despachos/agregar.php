...
} else if(in_array(98,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 

	$rs = pg_query($link, filtrar_sql("insert into confguia(id_cliente, nombre, codigo_principal, n_configuracion_01, n_configuracion_02, n_configuracion_03) values( $cli, '$nom', '$cod', '$conf1', '$conf2', '$conf3')"));
	if($rs){ 
		
		$rs = pg_query($link, filtrar_sql("select max(id_confguia) from confguia"));
		$rs = pg_fetch_array($rs);
		Auditoria("Agrego Configuracion Guia Despacho: $cod ",$rs[0]);

		$_SESSION['mensaje3']="Configuración de la Guía de Despacho Agregado";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro agregar la Configuración de la Guía de Despacho";
		Auditoria("Problema al registrar Configuración Guia de Despacho: ".pg_last_error($link),0);
	}

} // si validar
} else { 
	$cli = $cod = $nom = $conf1 = $conf2 = $conf3 = "";
	Auditoria("Accedio Al Modulo Agregar Configuracion Guia de Despachos",0);
...
val = false;
	if(document.getElementById('cod').value.length<1){ 
		mensaje("Debe indicar el Código Principal",1);
