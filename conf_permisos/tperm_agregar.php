...
if(empty($cli)){ $_SESSION['mensaje1']="Debe seleccionar un Cliente";
	...
	if(isset($_SESSION['mensaje1'])==false){ // si validar 2
		...
		$rs = pg_query($link, filtrar_sql("insert into tipo_permisos(id_cliente, id_responsable_general, dias_gestion, nombre, cant_doc, tipo_aviso, tipo_msj, esc, id_clasperm) 
		values ($cli, $resp, $dias, '$nom', $cant_doc, '$aviso', '$msj', $esc, $clas)"));
		if($rs){ 
	...
	$cli = $resp = 0;
	Auditoria("Accedio a Agregar Tipo de Permiso", 0);
}
...
<div class="header">Agregar Tipo de Permiso</div>
<form name="agregar" method="post" action="tperm_agregar.php" onsubmit="return validar();" enctype="multipart/form-data">
<fieldset>
<div class="fuelux">
...
<?php  
if(isset($_SESSION['treq']['req'])){ 
	$CantItems = count($_SESSION['treq']['req']);
	for($i=1; $i<=$CantItems; $i++){ echo "<script>agregar_detalle(".$_SESSION['treq']['req'][$i][0].",'".$_SESSION['treq']['req'][$i][1]."');</script>"; } 
} ?>
