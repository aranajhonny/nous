...
$_SESSION['acc']['mod'] = 17;
$_SESSION['acc']['form'] = 23;
include("../complementos/permisos.php");

if(isset($_REQUEST['lote'])){ $_SESSION['lote']=filtrar_campo('int', 6,$_REQUEST['lote']); }
...
} else if(in_array(216,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 

	$rs = pg_query($link, filtrar_sql("update lotes set nro='$nro', fecha_creacion='$tmp', cantidad=$cant, serial_inicio=$serial where id_lote = ".$_SESSION['lote']));
	if($rs){ 
		Auditoria("Actualizo Lote: $nro",$_SESSION['lote']);
		$_SESSION['mensaje3']="Lote Editado";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro editar el lote";
		Auditoria("Problema al actualizar el Lote Error: ".pg_last_error($link),$_SESSION['lote']);
	}

} // si validar

} else if(isset($_SESSION['lote'])){
$rs = pg_query($link, filtrar_sql("select * from lotes where id_lote = ".$_SESSION['lote']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico El Lote";
	Auditoria("Lote No Identificado ",$_SESSION['lote']);
	unset($_SESSION['lote']);
	header("location: listado.php");
	exit();
} else { 
	...
	$serial = $rs[3]; 
	$nro = $rs[4];
	Auditoria("Accedio Al Modulo Editar Lote: $nro",$_SESSION['lote']);
}

} else { 
	$_SESSION['mensaje1']="No se identifico el lote";
	Auditoria("Lote No Identificado",$_SESSION['lotes']);
...
<div class="header">Editar Lote<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
