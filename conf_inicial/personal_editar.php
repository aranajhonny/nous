...
if(isset($_REQUEST['pers'])){ $_SESSION['confini']['pers']=filtrar_campo('int', 6, $_REQUEST['pers']); }
...
$rs = pg_query($link, filtrar_sql("select count(id_personal) from personal where ci='$ci' and id_personal <> ".$_SESSION['confini']['pers']));
$rs = pg_fetch_array($rs);
if($rs[0]>0){ 
	$_SESSION['mensaje1']="numero de CI ya registrado";
} else { // si rif 

	$rs = pg_query($link, filtrar_sql("update personal set ci='$ci', nombre='$nom', telefono='$tlf', email='$correo', id_cargo=$cargo, id_area=$area, id_zona=$zona, tipo_aviso='$aviso' where id_personal = ".$_SESSION['confini']['pers']));
	if($rs==true || $rs>0){ 
		Auditoria("En Configuración Inicial Actualizo Los Datos del Personal: $ci $nom",$_SESSION['confini']['pers']);
		$_SESSION['mensaje3']="Datos del Personal Actualizados";
		header("location: personal_listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro actualizar los datos del Personal";
		Auditoria("Problema al actualizar los datos del Personal Error: ".pg_last_error($link),0);
	}
} // si rif
} // si validar

} else if(isset($_SESSION['confini']['pers'])){
$rs = pg_query($link, filtrar_sql("select * from personal where id_personal = ".$_SESSION['confini']['pers']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se identifico El Personal";
	Auditoria("En Configuración Inicial Personal No Identificado ",$_SESSION['confini']['pers']);
	header("location: personal_listado.php");
	exit();
} else {
	$rs = pg_fetch_array($rs);
	$cargo = $rs[1]; $ci = $rs[2]; $nom = $rs[3]; $tlf = $rs[4]; $correo = $rs[5];
	$area = $rs[7]; $zona = $rs[8]; $aviso = $rs[10];
	Auditoria("En Configuracion Inicial Accedio a Editar Datos del Personal: $ci $nom",$_SESSION['confini']['pers']);
}
...
<div class="header">Editar Datos del Personal</div>
<form name="editar" method="post" action="personal_editar.php" onsubmit="return validar();">
<fieldset>

<div class="form-group"><label>C.I.</label>
<input id="ci" name="ci" type="text" placeholder="Cédula de Identidad" class="form-control" maxlength="10" value="<?php echo $ci;?>" onkeypress="return permite(event,'ci')" onkeyup="mayu(this)" required/><p class="help-block">Ejemplo: V-20123456 o E-20123456</p></div>
