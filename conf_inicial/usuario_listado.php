...
Auditoria("En Configuración Inicial Accedio a Listado de Usuarios", 0);


if(isset($_REQUEST['eli'])){ 
	$id = filtrar_campo('int', 6, $_REQUEST['eli']);
	$rs = pg_query($link, filtrar_sql("delete from usuarios where id_usuario = $id"));
	if($rs){ 
		pg_query($link, filtrar_sql("delete from sys_acl where id_usu = $id"));
		Auditoria("En Configuración Inicial Usuario Eliminado",$id); 
		$_SESSION['mensaje3']="Usuario Eliminado";
	} else { 
		$_SESSION['mensaje1']="No se logro eliminar el usuario";
		Auditoria("Problema al eliminar el usuario Error: ".pg_last_error($link),$id);
	}
...
<div class="well">
<div class="header">Listado de Usuarios </div>
<table class="table table-striped table-bordered" id="managed-table">
	...
	<tbody>
<?php $rs = pg_query($link, filtrar_sql("select id_personal, ci, nombre, rif, razon_social from personal, clientes where personal.id_cliente = clientes.id_cliente and clientes.id_cliente = ".$_SESSION['confini']['cli']." order by ci desc ")); 
$r = pg_num_rows($rs);
if($r!=false && $r>0){ $_SESSION['confini']['paso'] = 5;
...
<?php if($id==0){ ?>
<div class=" info-tooltip"><img src="../img/plus.png" width="15" height="15" title="Generar Usuario" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="location.href='usuario_agregar.php?usu=<?php echo $r[0];?>'" /></div>
<?php } else { ?>
<div class=" info-tooltip">
<img src='../img/cross.png' width='15' height='15'  title='Eliminar Usuario' rel='tooltip' data-placement='right' onclick="pregunta(<?php echo $r[0];?>);"/>
<img src="../img/pencil.png" width="15" height="15" title="Editar Datos del Usuario" style="margin-left:15px;"  rel="tooltip" data-placement="right" onclick="location.href='usuario_editar.php?usu=<?php echo $r[0];?>'" /></div>
<?php } ?>
</div></td>
         ...
         vex.dialog.open({
             ...
             buttons: [
                 $.extend({}, vex.dialog.buttons.YES, {
                     text: 'SI',
					 click: function(){ location.href='usuario_listado.php?eli='+id; }
                 }),
...
<?php 
if($_SESSION['confini']['paso']==5) { echo "<script>window.open('asignar.php','asigs');</script>";
} else { echo "<script>window.open('desabilitado.php?id=6','asigs');</script>"; }?>
