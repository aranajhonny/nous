...
if(isset($_REQUEST['eli'])){ 
	...
	if($rs){ 
		Auditoria("En Configuración Inicial Personal Eliminado",$id); 
		$_SESSION['mensaje3']="Personal Eliminado";
	} else { 
		$_SESSION['mensaje1']="No se logro eliminar el personal";
		Auditoria("Problema al eliminar el Personal Error: ".pg_last_error($link),$id);
	}
	
} else {
	Auditoria("En Configuración Inicial Accedio a Listado de Personal", 0);
}
...
$r = pg_num_rows($rs);
if($r!=false && $r>0){ $_SESSION['confini']['paso']=4;
...
<th class="extra2"><input type="text" name="ci" id="ci" placeholder="Buscar Cédula de Identidad" class="search_init"  style="width:160px;" /></th>
...
<?php 
if($_SESSION['confini']['paso']==4 && isset($_SESSION['confini']['usus'])==false) { echo "<script>window.open('usuario_listado.php','usus');</script>";
} else if($_SESSION['confini']['paso']==4){ 
} else { echo "<script>window.open('desabilitado.php?id=5','usus');</script>"; }?>
