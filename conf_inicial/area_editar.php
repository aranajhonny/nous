...
if(isset($_REQUEST['area'])){ $_SESSION['confini']['area']=filtrar_campo('int', 6, $_REQUEST['area']); }
...
if(isset($_POST['guardar'])){ 
	...
	$rs = pg_query($link, filtrar_sql("update areas set id_dependencia=$dep, descripcion='$des' where id_area = ".$_SESSION['confini']['area']));
		if($rs==true || $rs>0){ 
	
			Auditoria("En Configuración Inicial Se Actualizaron Los Datos del Área: $des",$_SESSION['confini']['area']); 
	
			$_SESSION['mensaje3']="Datos del Área Actualizados";
			$_SESSION['confini']['area'] = true;
			header("location: area_listado.php");
			exit();
		} else { 
			$_SESSION['mensaje1']="No se logro Actualizar Los Datos del Área";
			Auditoria("Problema al Actualizar los Datos del Área Error: ".pg_last_error($link),0);
		}
	
	} // si validar
	
} else if(isset($_SESSION['confini']['area'])){
$rs = pg_query($link, filtrar_sql("select * from areas where id_area = ".$_SESSION['confini']['area']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	$_SESSION['mensaje1']="No se Identifico el Área";
	Auditoria("Área No Identificada ",$$_SESSION['confini']['area']);
	header("location: area_listado.php");
	exit();
} else { 
	...
	$des = $rs[2]; 
	$dep = $rs[3];
	Auditoria("En Configuración Inicial Accedio a Editar Datos del Áreas: $des",$$_SESSION['confini']['area']);
}
...
<div class="header">AEditar Datos del Área</div>
<form name="agregar" method="post" action="area_editar.php" onsubmit="return validar();">
<fieldset>
