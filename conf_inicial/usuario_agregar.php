...
if(isset($_REQUEST['usu'])){ 
	$_SESSION['confini']['usu'] = filtrar_campo('int', 6, $_REQUEST['usu']); 
	$rs = pg_query($link, filtrar_sql("select id_area, id_zona, id_confunid from personal where id_personal = ".$_SESSION['confini']['usu']));
...
$pers = filtrar_campo('int', 6, $_SESSION['confini']['usu']);
...
if($rs){ 
	...
	Auditoria("En Configuración Inicial Agrego Usuario: $nom",$pers);
...
$rs = pg_query($link, filtrar_sql("insert into sys_acl(id_acc,id_usu,permiso)values(".$_POST["chk_".$i].",$pers,TRUE);"));
	...
	Auditoria("En Configuración Inicial Accedio a Agregar Usuario",0);
}
...
<form name="agregar" method="post" action="usuario_agregar.php" onsubmit="return validar();">
<fieldset>
<div class="fuelux">
...
<input id="clave" name="clave" type="password" placeholder="Clave de Usuario" class="form-control" maxlength="20" value="" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="Debe llevar Mayúsculas, minúsculas, números; y un minimo minimo 8 caracteres" /></div>
...
function validar(){ 
	...
	} else if(document.getElementById('clav').value.length<8){ mensaje("Debe indicar la clave de usuario y debe contener al menos 8 digitos",1);
