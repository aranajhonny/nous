...
} else if(in_array(227,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 

$rs = pg_query($link, filtrar_sql("select count(id_cliente) from clientes where rif='$rif' and id_cliente <> ".$_SESSION['cliente']));
$rs = pg_fetch_array($rs);
if($rs[0]>0){ 
	$_SESSION['mensaje1']="numero de rif ya registrado";

} else { // si rif 
	$rs = pg_query($link, filtrar_sql("update clientes set rif='$rif', razon_social='$razon', direccion='$dir', telefono='$tlf', contacto='$contacto', correo_contacto='$correo', id_tipo_cliente=$tipo   where id_cliente = ".$_SESSION['cliente']));
	if($rs){ 
	
		Auditoria("Actualizo Cliente: $rif $razon",$_SESSION['cliente']);
		
		$_SESSION['mensaje3']="Datos del Cliente Editados";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro editar los datos del cliente";
		Auditoria("Problema al actualizar el cliente Error: ".pg_last_error($link),0);
...
if($r==false || $r<1){ 
	...
	$contacto = $rs[6]; 
	$correo = $rs[7];
	Auditoria("Accedio Al Modulo Editar Cliente: $rif $razon",$_SESSION['cliente']);
}
...
<div class="header">Editar Cliente<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
