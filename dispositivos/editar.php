...
} else if(in_array(217,$_SESSION['acl'])==false){$_SESSION['mensaje1']= "no posee permiso para guardar este registro";
} else { // si validar 

	$rs = pg_query($link, filtrar_sql("update dispositivos set id_tipo_disp=$tipo, id_cliente=$cli, id_lote=$lote, serial='$serial', estatus='$est' where id_dispositivo = ".$_SESSION['dispositivo']));
	if($rs){
		
		Auditoria("Actualizo Dispositivos: $serial",$_SESSION['dispositivo']); 
		$_SESSION['mensaje3']="Dispositivo Editado";
		header("location: listado.php");
		exit();
	} else { 
		$_SESSION['mensaje1']="No se logro editar el dispositivo";
		Auditoria("Problema al actualizar el dispositivo Error: ".pg_last_error($link),$_SESSION['dispositivo']);
	}

} // si validar
} else if(isset($_SESSION['dispositivo'])){
$rs = pg_query($link, filtrar_sql("select * from dispositivos where id_dispositivo=".$_SESSION['dispositivo']));
$r = pg_num_rows($rs);
if($r==false || $r<1){ 
	...
	$serial = $rs[4]; 
	$est = $rs[5];
	Auditoria("Accedio Al Modulo Editar Dispositivos: $serial",$_SESSION['dispositivo']);
}
...
<div class="header">Editar Dispositivo<a class="headerclose"><i class="fa fa-times pull-right"></i></a> <a class="headerrefresh"><i class="fa fa-refresh pull-right"></i></a> <a class="headershrink"><i class="fa fa-chevron-down pull-right"></i></a></div>
